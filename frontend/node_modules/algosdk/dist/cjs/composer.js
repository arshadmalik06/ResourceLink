...
const makeTxn_js_1 = require("./makeTxn.js");
...
class AtomicTransactionComposer {
    ...
    clone() {
        ...
        theClone.transactions = this.transactions.map(({ txn, signer }) => {
            ...
            return {
                ...
                txn: transaction_js_1.Transaction.fromEncodingData(txnMap),
                signer,
            };
        });
    ...
    addMethodCall({ appID, method, methodArgs, sender, suggestedParams, onComplete, approvalProgram, clearProgram, numGlobalInts, numGlobalByteSlices, numLocalInts, numLocalByteSlices, extraPages, appAccounts, appForeignApps, appForeignAssets, boxes, access, note, lease, rekeyTo, rejectVersion, signer, }) {
        ...
        else if (onComplete === base_js_1.OnApplicationComplete.UpdateApplicationOC) {
        ...
        for (let i = 0; i < methodArgs.length; i++) {
            ...
            if ((0, index_js_1.abiTypeIsTransaction)(argType)) {
                if (!(0, signer_js_1.isTransactionWithSigner)(argValue) ||
                    !(0, index_js_1.abiCheckTransactionType)(argType, argValue.txn)) {
            ...
            if ((0, index_js_1.abiTypeIsReference)(argType)) {
                ...
                argType = new index_js_1.ABIUintType(8);
            }
        ...
        const foreignApps = appForeignApps == null ? [] : appForeignApps.map(utils_js_1.ensureUint64);
        const foreignAssets = appForeignAssets == null ? [] : appForeignAssets.map(utils_js_1.ensureUint64);
        for (let i = 0; i < refArgTypes.length; i++) {
            const refType = refArgTypes[i];
            const refValue = refArgValues[i];
            let resolved = 0;
            switch (refType) {
                case index_js_1.ABIReferenceType.account: {
                    const addressType = new index_js_1.ABIAddressType();
                ...
                case index_js_1.ABIReferenceType.application: {
                    const uint64Type = new index_js_1.ABIUintType(64);
                    ...
                    resolved = populateForeignArray(refAppID, foreignApps, (0, utils_js_1.ensureUint64)(appID));
                    break;
                }
                case index_js_1.ABIReferenceType.asset: {
                    const uint64Type = new index_js_1.ABIUintType(64);
        ...
        if (basicArgTypes.length > MAX_APP_ARGS - 1) {
            ...
            basicArgTypes.push(new index_js_1.ABITupleType(lastArgTupleTypes));
        ...
        const appCall = {
            txn: (0, makeTxn_js_1.makeApplicationCallTxnFromObject)({
                sender,
                appIndex: appID,
                ...
                access,
                onComplete: onComplete == null ? base_js_1.OnApplicationComplete.NoOpOC : onComplete,
                approvalProgram,
                clearProgram,
                numGlobalInts,
                numGlobalByteSlices,
    ...
    buildGroup() {
        if (this.status === AtomicTransactionComposerStatus.BUILDING) {
            ...
            if (this.transactions.length > 1) {
                (0, group_js_1.assignGroupID)(this.transactions.map((txnWithSigner) => txnWithSigner.txn));
            }
    ...
    async gatherSignatures() {
        ...
        const txIDs = signedTxns.map((stxn, index) => {
            try {
                return encoding.decodeMsgpack(stxn, signedTransaction_js_1.SignedTransaction).txn.txID();
            }
            catch (err) {
    ...
    async simulate(client, request) {
        ...
        const stxns = await this.gatherSignatures();
        const txnObjects = stxns.map((stxn) => encoding.decodeMsgpack(stxn, signedTransaction_js_1.SignedTransaction));
        const currentRequest = request == null ? new types_js_1.SimulateRequest({ txnGroups: [] }) : request;
        currentRequest.txnGroups = [
            new types_js_1.SimulateRequestTransactionGroup({
                txns: txnObjects,
            }),
        ];
    ...
    async execute(client, waitRounds) {
        ...
        const confirmedTxnInfo = await (0, wait_js_1.waitForConfirmation)(client, txIDs[indexToWaitFor], waitRounds);
    ...
    static parseMethodResponse(method, methodResult, pendingInfo) {
        ...
        try {
            ...
            if (method.returns.type !== 'void') {
                ...
                if (logs.length === 0) {
                    throw new Error(`App call transaction did not log a return value ${(0, utils_js_1.stringifyJSON)(pendingInfo)}`);
                }
                const lastLog = logs[logs.length - 1];
                if (lastLog.byteLength < 4 ||
                    !(0, utils_js_1.arrayEqual)(lastLog.slice(0, 4), RETURN_PREFIX)) {
                    throw new Error(`App call transaction did not log a ABI return value ${(0, utils_js_1.stringifyJSON)(pendingInfo)}`);
                }
