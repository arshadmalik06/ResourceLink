...
exports.pq25WordMnemonicToSeed = exports.masterDerivationKeyToMnemonic = exports.mnemonicToMasterDerivationKey = exports.secretKeyToMnemonic = exports.mnemonicToSecretKey = exports.seedFromMnemonic = exports.mnemonicFromSeed = exports.NOT_IN_WORDS_LIST_ERROR_MSG = exports.FAIL_TO_DECODE_MNEMONIC_ERROR_MSG = void 0;
/* eslint-disable no-bitwise */
const english_js_1 = __importDefault(require("./wordlists/english.js"));
...
function applyWords(nums) {
    return nums.map((n) => english_js_1.default[n]);
}
...
function seedFromMnemonic(mnemonic) {
    ...
    for (const w of key) {
        if (english_js_1.default.indexOf(w) === -1)
            throw new Error(exports.NOT_IN_WORDS_LIST_ERROR_MSG);
    }
    const checksum = words[words.length - 1];
    const uint11Array = key.map((word) => english_js_1.default.indexOf(word));
...
function mnemonicToSecretKey(mn) {
    ...
    const addr = new address_js_1.Address(keys.publicKey);
 ...
 */
function pq25WordMnemonicToSeed(mn, scheme) {
    if (scheme.length !== address_js_1.PQ_SCHEME_SIZE) {
        throw new Error(`invalid PQ scheme length: expected ${address_js_1.PQ_SCHEME_SIZE} bytes, got ${scheme.length}`);
    }
    return new Uint8Array(nacl.genericHash((0, utils_js_1.concatArrays)(PQ_KEY_PREFIX, scheme, seedFromMnemonic(mn))));
}
exports.pq25WordMnemonicToSeed = pq25WordMnemonicToSeed;
