...
exports.appendSignRawMultisigSignature = exports.appendSignMultisigTransactionWithSigner = exports.appendSignMultisigTransaction = exports.signMultisigTransactionWithSigner = exports.signMultisigTransaction = exports.mergeMultisigTransactions = exports.createMultisigTransaction = exports.MULTISIG_SIGNATURE_LENGTH_ERROR_MSG = exports.MULTISIG_USE_PARTIAL_SIGN_ERROR_MSG = exports.MULTISIG_NO_MUTATE_ERROR_MSG = exports.MULTISIG_MERGE_SIG_MISMATCH_ERROR_MSG = exports.MULTISIG_MERGE_WRONG_PREIMAGE_ERROR_MSG = exports.MULTISIG_MERGE_MISMATCH_AUTH_ADDR_MSG = exports.MULTISIG_MERGE_MISMATCH_ERROR_MSG = exports.MULTISIG_MERGE_LESSTHANTWO_ERROR_MSG = void 0;
...
async function partialSignTxnWithSigner(transaction, { version, threshold, pks }, txnSigner) {
    const { stxn } = await (0, signing_js_1.signTransactionWithSigner)(transaction, txnSigner);
    const rawSig = stxn.sig;
    if (rawSig === undefined) {
        throw Error(`Expected TransactionSigner to return a signed transaction with a sig, but sig was empty`);
    }
 ...
 */
async function signMultisigTransactionWithSigner(txn, { version, threshold, addrs }, txnSigner) {
    const pks = (0, multisig_js_1.pksFromAddresses)(addrs);
    const blob = await partialSignTxnWithSigner(txn, { version, threshold, pks }, txnSigner);
    return {
        txID: txn.txID(),
        blob,
    };
}
exports.signMultisigTransactionWithSigner = signMultisigTransactionWithSigner;
/**
 ...
 * @returns an object with txID and blob properties
 */
async function appendSignMultisigTransactionWithSigner(multisigTxnBlob, { version, threshold, addrs }, txnSigner) {
    const pks = (0, multisig_js_1.pksFromAddresses)(addrs);
    // obtain underlying txn, sign it, and merge it
    const multisigTxObj = encoding.decodeMsgpack(multisigTxnBlob, signedTransaction_js_1.SignedTransaction);
    const partialSignedBlob = await partialSignTxnWithSigner(multisigTxObj.txn, { version, threshold, pks }, txnSigner);
    return {
        txID: multisigTxObj.txn.txID(),
        blob: mergeMultisigTransactions([multisigTxnBlob, partialSignedBlob]),
    };
}
exports.appendSignMultisigTransactionWithSigner = appendSignMultisigTransactionWithSigner;
/**
