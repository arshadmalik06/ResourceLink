 ...
 */
export function pq25WordMnemonicToSeed(mn, scheme) {
    if (scheme.length !== PQ_SCHEME_SIZE) {
        throw new Error(`invalid PQ scheme length: expected ${PQ_SCHEME_SIZE} bytes, got ${scheme.length}`);
    }
    return new Uint8Array(nacl.genericHash(concatArrays(PQ_KEY_PREFIX, scheme, seedFromMnemonic(mn))));
}
