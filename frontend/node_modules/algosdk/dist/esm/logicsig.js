...
import { Address, addressFromPQSig, isValidAddress, } from './encoding/address.js';
...
import { encodedMultiSigToEncodingData, encodedMultiSigFromEncodingData, ENCODED_MULTISIG_SCHEMA, ENCODED_PQSIG_SCHEMA, encodedPQSigToEncodingData, encodedPQSigFromEncodingData, } from './types/transactions/encoded.js';
...
export class LogicSig {
    ...
    verify(publicKey) {
        ...
        if (this.lmsig) {
            ...
            const lmsigProgram = utils.concatArrays(MSIG_PROGRAM_TAG, multisigAddr.publicKey, this.logic);
    ...
    async signWithSigner(signer, msig) {
        ...
        if (msig == null) {
            ...
            if (lmsig.v !== msig.version ||
                lmsig.thr !== msig.threshold ||
                lmsig.subsig.length !== expectedPks.length ||
                !lmsig.subsig.every((subsig, i) => utils.arrayEqual(subsig.pk, expectedPks[i]))) {
    ...
    async appendToMultisigWithSigner(signer) {
        ...
        if (sigResult.lmsig.v !== this.lmsig.v ||
        ...
        for (const subsig of sigResult.lmsig.subsig) {
            if (subsig.s) {
                ...
                const thisSubsig = this.lmsig.subsig.find((s) => utils.arrayEqual(s.pk, subsig.pk));
    ...
    signProgramMultisig(secretKey, msig) {
        ...
        const toBeSigned = utils.concatArrays(MSIG_PROGRAM_TAG, multisigAddr.publicKey, this.logic);
        const sig = nacl.sign(toBeSigned, secretKey);
        return sig;
    }
    /**
