 ...
 */
export async function signMultisigTransactionWithSigner(txn, { version, threshold, addrs }, txnSigner) {
    const pks = pksFromAddresses(addrs);
 ...
 * @returns an object with txID and blob properties
 */
export async function appendSignMultisigTransactionWithSigner(multisigTxnBlob, { version, threshold, addrs }, txnSigner) {
    const pks = pksFromAddresses(addrs);
    // obtain underlying txn, sign it, and merge it
    const multisigTxObj = encoding.decodeMsgpack(multisigTxnBlob, SignedTransaction);
